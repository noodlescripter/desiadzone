...
app.post('/ads/posting', async function (req, res) {
    try {
        const { userId, title, address, city, state, zipCode, description, category, phoneNumber, price, userName, email } = req.body;
        ...
        if (productDetails) {
            ...
            if (photos) {
                for (const p of photos) {
                    try{
                        console.log(p.imageKey)
                        await cloudinary.uploader.destroy(p.imageKey);
                    } catch(error){
                        console.log('cloudinary issues', error)
                    }
                }
            }
...
app.post(`/ads/:id/manageAds/updatestatus/sold`, async function (req, res) {
    try {
        const { id, status } = req.params;
        if (!id)
            return;
        console.log(req.params);
        const updateRes = await Product.updateOne({ _id: id },
            {
                status: "sold"
            });
        if (!updateRes)
