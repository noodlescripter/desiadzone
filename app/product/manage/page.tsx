...
export default function Component() {
      ...
      if (userSessionData.userId) {
        try {
          const res = await axios.get(
            `${process.env.NEXT_PUBLIC_BACK_END_URL}/ads/${userSessionData.userId}/manageAds`,
          );
          setUserAds(res.data);
  ...
  async function deleteRequestedProduct(productId: String) {
    try {
      ...
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_BACK_END_URL}/ads/${productId}/manageAds/delete`,
      );
  ...
  async function handleSold(productId: String) {
    try {
      ...
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_BACK_END_URL}/ads/${productId}/manageAds/updatestatus/sold`,
      );
      if (!res) {
        setAlert({
          show: true,
          desc: "Could not update the ad",
          variant: "error",
          title: "Update Error",
        });
      }
      ...
      <div className="flex items-center justify-between m-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink className="p-2" href="/adlisting">
                <ListIcon></ListIcon>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href={`/product/manage`}>
                Manage Ad
              </BreadcrumbLink>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        <Link href="/postlisting" prefetch={false}>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogCancel
                          ...
                          onClick={() => deleteRequestedProduct(ad._id)}
                        >
                          Delete
                        </AlertDialogCancel>
                      </AlertDialogFooter>
